...
const boxA = [];
for (let i = 0; i < 4; i++) {
    let ball = new Ball(Math.random()*700, Math.random()*700, 10);
    ball.f = 0.05;
...
// 添加桌面边缘碰撞规则
const bound = new Engine.Bound(100, 100, 1000, 550, 0.9);
const PLAY_ZONE = [100, 100, 1000, 550];
const RESTITUTION = 0.9; // 碰撞恢复系数
hog.addLaw(() => {
    hog.between(boxA, [bound]).do((A, B) => hog.elasticImpact(A, B));
});
